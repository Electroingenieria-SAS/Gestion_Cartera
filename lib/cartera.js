...
export async function getResumenCliente(nit) {
  ...
  const { data: docs } = await supabase
    .from("cartera_documentos")
    .select("saldo, categoria, dias_vencidos, nombre_cliente, ciudad, vendedor, tipo_docto, numero_docto, fecha_docto, fecha_vencimiento, condicion_pago")
  ...
  for (const d of lista) {
   ...
   if (s > 0) dias = Math.max(dias, parseInt(d.dias_vencidos) || 0);
  }
  const base = lista[0] || {};
  ...
  return {
    ...
    condicion_pago: base.condicion_pago || null,
