...
import { getNitsEnJuridico } from "./juridico";
...
export async function getAlertas() {
  ...
  // Clientes que ya están en cobro jurídico: los maneja jurídico, no la auxiliar.
  const enJuridico = await getNitsEnJuridico();
  // Un cliente se oculta si ya lo gestionó hoy, tiene compromiso a futuro, o está en jurídico.
  const ocultar = (nit) =>
    gestionadosHoy.has(String(nit)) ||
    compromisoVigente.has(String(nit)) ||
    enJuridico.has(String(nit));
  ...
  for (const a of acu || []) {
    // Si ya lo gestionó hoy o está en cobro jurídico, no genera alerta en cartera.
    if (gestionadosHoy.has(String(a.cliente_nit))) continue;
    if (enJuridico.has(String(a.cliente_nit))) continue;
